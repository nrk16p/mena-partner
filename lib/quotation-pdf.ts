...
export function quotationDocDef(q: Quotation): any {
    ...
    table: { widths: ["*"], body: [[
      {
        ...
        stack: [
          ...
          { text: seg(bigName || "-"), bold: true, fontSize: 14, color: INK, margin: [0, 5, 0, 4] },
          ...rows.map(([k, v]) => ({ columns: [
            { text: seg(k), color: "#a1a1aa", fontSize: 11, width: 50 },
            { text: seg(v || "-"), color: "#3f3f46", fontSize: 13, width: "*" },
          ], margin: [0, 1, 0, 1] })),
        ],
      },
    ]] },
  ...
  const priceRow = (label: string, value: string, opts: { big?: boolean; gold?: boolean } = {}) => [
    { text: seg(label), fontSize: opts.big ? 14 : 13, bold: opts.big, color: opts.big ? INK : "#52525b", margin: [0, opts.big ? 3 : 1.5, 0, opts.big ? 3 : 1.5] },
    { text: value, alignment: "right", fontSize: opts.big ? 15 : 13, bold: opts.big || opts.gold, color: opts.gold ? GOLD_DK : INK, margin: [0, opts.big ? 3 : 1.5, 0, opts.big ? 3 : 1.5] },
  ]
  ...
  return {
    ...
    defaultStyle: { font: "Sarabun", fontSize: 13, lineHeight: 1.1 },
      ...
      // ── หน้า 1: หัวเอกสารเต็ม — โลโก้บรรทัดบน · ชื่อบริษัท | บล็อกเลขที่ใบเสนอ (ขวา) ──
      {
        columns: [
          ...
          },
          {
            width: 168,
            stack: [
              { text: "ใบเสนอราคา", bold: true, fontSize: 18, color: GOLD_DK, alignment: "right" },
              { text: "QUOTATION", fontSize: 10, color: GOLD, alignment: "right", characterSpacing: 2 },
              { text: seg(`เลขที่ ${q.quotationNo}`), fontSize: 13, bold: true, color: INK, alignment: "right", margin: [0, 4, 0, 0] },
              { text: seg(`วันที่ ${thDate(q.createdAt)}`), fontSize: 11, color: "#52525b", alignment: "right" },
              // สถานะ (ป้ายสี)
            ...
            stack: [{
              columns: [
                { width: "*", stack: [
                  ...
                  { text: `${fm(q.monthlyPayment)} บาท`, color: GOLD_DK, bold: true, fontSize: 26, margin: [0, 2, 0, 0] },
                ] },
                { width: "auto", stack: [
